...
<div class="container mt-3">
    <div class="row justify-content-center g-5">
        ...
        <div class="col-md-10">
            <div class="row">
                <div class="col-md-8">
                    <h1>{{ __('Artikel aanmaken') }}</h1>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                <form class="row g-3" action="{{ route('admin.articles.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <textarea class="form-control" name="content" id="content" rows="5" placeholder="Bij je lokale autodealer krijg je nu..." value="{{ $article->content ?? '' }}"></textarea>
                        </div>

                        <div class="row">
                            <div class="col-6">
                                <label for="date">Publicatie datum</label>
                                <input type="date" id="date" name="date" class="form-control" value="{{ isset($article) ? $article->date->format('d-m-Y') : '' }}" required>
                            </div>

                            <div class="col-6">
                                <label for="ExpDate">Verval datum</label>
                                <input type="date" id="ExpDate" name="ExpDate" class="form-control" value="{{ isset($article) ? $article->ExpDate->format('d-m-Y') : '' }}" required>
                            </div>
                        </div>
